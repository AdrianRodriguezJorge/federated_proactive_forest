...
def render() -> None:
    """Renders page_ranking Streamlit page displaying all selected trees."""
    ...
    if not results:
        st.warning(
            "⚠️ Ejecuta primero el experimento en la página '▶️ Ejecutar'."
        )
        return

    # Get dynamic threshold from config
    agg_cfg = cfg.get("aggregation", {}) if cfg else {}
    ...
    n_sel = sum(
        1
        for e in results.all_tree_entries
    ...
    if sid not in ["s1_simple_pool", "s1"] and has_logs:
        ...
        if results.convergence_round is not None:
            ...
            if 0 < last_idx < len(logs):
                ...
                st.success(
                    f"🎯 **Convergencia alcanzada en el episodio "
                    f"{results.convergence_round}**.\n\n"
                    f"La mejora final fue de `{improvement:.5f}` "
                    f"(Umbral: `{conv_thr}`)."
                )
            else:
                st.success(
                    f"🎯 **Parada Temprana**: El modelo global convergió "
                    f"en el episodio {results.convergence_round}."
                )
        else:
            # Did not converge
            if len(logs) >= 2:
                ...
                st.warning(
                    f"⚠️ **Selección completa ({n_sel} árboles)**.\n\n"
                    f"No se alcanzó la convergencia. Variación última: "
                    f"`{improvement:.5f}` (Umbral: `{conv_thr}`)."
                )
            else:
                st.warning(
                    f"⚠️ **Sin Convergencia**: Se seleccionaron todos "
                    f"los árboles candidatos ({n_sel}). No hubo "
                    f"suficientes episodios para evaluar la mejora."
                )
    else:
        st.info(info_text)
    ...
    st.caption(
        "El ID del árbol tiene formato `{client_id}_tree{local_id}`. "
        "Este `local_id` es el que usa el No-Repeat Merge en la "
    ...
    for rank, entry in enumerate(results.all_tree_entries, 1):
        sel = entry.tree_local_id in results.selected_ids.get(
            str(entry.client_id), []
        )
        rows.append(
            {
    ...
    if sid in ("s1_simple_pool", "s1"):
        df = df.sort_values(
            ["Cliente", "Rank"], ascending=[True, True]
    ...
    pct_text = (
        f"{100 * n_sel_shown / n_shown:.1f}%" if n_shown else "—"
    )
    cD.metric("% Selección", pct_text)
    ...
    if not sel_clients:
        ...
        selected_cid = st.selectbox(
            "Seleccionar cliente para ver sus árboles",
        ...
        if selected_cid == "Todos":
            ...
            n_s = sub["_sel"].sum()
            st.write(
                f"Mostrando **{len(sub)}** árboles del cliente "
                f"**{selected_cid}** ({int(n_s)} seleccionados)."
            )
            st.dataframe(
                sub[display_cols].reset_index(drop=True),
                use_container_width=True,
            )
