...
def render() -> None:
    """Renders page_metrics Streamlit page with comparative and local KPIs."""
    ...
    if is_s9:
        ...
        if report is None:
            st.error(
                f"No se encontraron reportes métricos para el "
                f"cliente '{cid}'."
            )
            return

        title = f"Modelo Local — {cid}"
        dict_key = int(cid) if cid.isdigit() else cid
        meta = results.client_metadata.get(cid) or results.client_metadata.get(
            dict_key
        )
    else:
        options = ["🌐 Modelo Global"] + [
            f"👤 {cid}" for cid in results.client_ids
        ]
        ...
        if sel == "🌐 Modelo Global":
            ...
            dict_key = int(cid) if cid.isdigit() else cid
            meta = results.client_metadata.get(
                cid
            ) or results.client_metadata.get(dict_key)
    ...
    if meta:
        if isinstance(meta, dict):
            ...
            st.caption(
                f"Árboles locales entrenados: {n_trees} | "
    ...
    c5.metric(
        "PCD (diversidad)",
        f"{report.pcd:.4f}",
        help="Percentage Correct Diversity — Zona de diversidad (10%-90%).",
    )
    ...
    try:
        ...
        st.dataframe(
            df_cm.style.background_gradient(cmap="Blues"),
    ...
    per_df = pd.DataFrame(
        {
            "Clase": class_names,
            "Precision": [
                round(report.per_class_prec[c], 4) for c in class_names
            ],
            "Recall": [
                round(report.per_class_recall[c], 4) for c in class_names
            ],
            "F1-Score": [
                round(report.per_class_f1[c], 4) for c in class_names
            ],
        }
    )
    ...
    try:
        ...
        melted = per_df.melt(
            id_vars="Clase", var_name="Métrica", value_name="Valor"
        )
        fig2 = px.bar(
            melted,
            x="Clase",
