...
class FedDataDistributor:
    """
    Handles the distribution of dataset splits across clients for federated learning.
    """
    ...
    def distribute(self, dataset_split: DatasetSplit, seed: int = 42) -> Tuple[DatasetSplit, Any]:
        ...
        if distribution_type == 'iid':
            federated_data = FedDataDistribution.iid_distribution(centralized_dataset, n_nodes=n_clients)
        else:
            ...
            federated_data = FedDataDistribution.from_config(centralized_dataset, config)
