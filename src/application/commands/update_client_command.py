...
class UpdateClientCommand:
    """
    Command to update clients with the global aggregated model using No-Repeat Merge.
    Concatenates local trees with global trees, excluding local trees already selected in aggregation.
    """
    ...
    def execute_on_client(self, client_data: Dict[str, Any], global_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Concatenates local trees with global trees, excluding selected local trees to avoid redundancy.
        ...
        # Calculate selected_local_ids: local indices of client's trees selected in global
        selected_local_ids = []
        ...
        merged_trees = ClientUpdater.merge(local_trees, global_trees, selected_local_ids)

        # Update client's model with merged trees
        from src.domain.model.proactive_forest import ProactiveForest
        class_names = global_data.get('class_names', None)
        updated_forest = ProactiveForest.from_trees(merged_trees, class_names=class_names)
