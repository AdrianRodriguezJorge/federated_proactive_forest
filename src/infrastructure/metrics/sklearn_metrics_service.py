...
class SklearnMetricsService(IMetricsService):
    ...
    def f1_score(self, y_true: np.ndarray, y_pred: np.ndarray, average: str = 'macro') -> Any:
        res = metrics.f1_score(y_true, y_pred, average=average, zero_division=0)
    ...
    def precision_score(self, y_true: np.ndarray, y_pred: np.ndarray, average: str = 'macro') -> Any:
        res = metrics.precision_score(y_true, y_pred, average=average, zero_division=0)
        return float(res) if average is not None else res

    def recall_score(self, y_true: np.ndarray, y_pred: np.ndarray, average: str = 'macro') -> Any:
        res = metrics.recall_score(y_true, y_pred, average=average, zero_division=0)
        return float(res) if average is not None else res
