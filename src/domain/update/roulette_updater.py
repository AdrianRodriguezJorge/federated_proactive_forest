...
Applies the β-fusion formula to blend a client's local roulette
with the server's global roulette:

    Roulette_new = β · Roulette_local + (1 − β) · Roulette_global

When β = 0 (default), the client fully adopts the global roulette,
ensuring strong federation-wide coordination.
When β = 1, the client ignores the global signal entirely.
Intermediate values balance local specialisation with global consensus.
"""
...
class RouletteUpdater:
    ...
    def __init__(self, beta: float = 0.0):
        """
        Args:
            beta: Blending coefficient in [0, 1].
                  0 → full global adoption (default, as specified in the proposal).
    ...
    def fuse(
        ...
        Args:
            local_roulette:  1-D array of shape (n_features,) from the client.
            global_roulette: 1-D array of shape (n_features,) from the server.

        Returns:
            np.ndarray: Fused probability vector, normalised to sum to 1.0.
