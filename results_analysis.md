...
2. **Efectividad de la Selección Proactiva Híbrida**: La estrategia **S7 (`s7_perclient_f1_pcd`)** se posiciona como la mejor configuración global con una exactitud (Accuracy) media de **0.8953**, seguida de cerca por su equivalente global **S4 (`s4_global_f1_pcd`)** con un Accuracy medio de **0.8915**. Esto confirma empíricamente la hipótesis de diseño del framework: incorporar la diversidad de los árboles (medida mediante *Pairwise Class Diversity* - PCD) junto al rendimiento de clasificación en el criterio de selección enriquece la generalización del ensamble y previene la redundancia de estimadores.
3. **Eficiencia en Comunicación de la Ruleta de Atributos (S8)**: Aunque las variantes de la estrategia S8 muestran un Accuracy ligeramente inferior a la línea base local (~0.845 vs 0.858), logran un rendimiento altamente competitivo sincronizando únicamente vectores de probabilidad de importancia de atributos locales, reduciendo drásticamente la sobrecarga de transmisión de modelos estructurados.
...
### 2.1. Rendimiento en Exactitud (Accuracy)

La siguiente tabla presenta el accuracy promedio obtenido por cada estrategia a través de los folds experimentales para cada dataset, ordenadas de mayor a menor según su promedio global.
...
### 2.2. Diversidad del Ensamble (PCD)
...
graph TD
    ...
    B --> B1["S7: Selección Per-Cliente (F1 + PCD)<br>Mejor Generalización (Accuracy: 0.8953)"]
    B --> B2["S1: Pool Completo (Sin Selección)<br>Sufre de Redundancia y Ruido (Accuracy: 0.8809)"]
    
    C --> C1["Sincronización por Importancia de Atributos<br>Alta Eficiencia en la Transferencia de Información"]
    
    D --> D1["Línea Base (Accuracy: 0.8578)<br>Falta de Colaboración Limita el Rendimiento"]
```

### 3.1. Eficacia de la Selección Proactiva frente al Pool Completo (S2-S7 vs S1)
La estrategia simple de agregación de pools sin cribado (**S1**) alcanza un Accuracy medio de **0.8809**. Al incorporar mecanismos de filtrado basados en el rendimiento y la diversidad en el servidor (S2–S7), el rendimiento medio asciende notablemente (hasta **0.8953** en S7).
...
* **Criterios de selección**: Las estrategias S7 y S4, que seleccionan estimadores utilizando una métrica híbrida de rendimiento y diversidad, superan a las estrategias basadas únicamente en rendimiento predictivo (como S5 y S2) o sin selección (S1). El rendimiento puramente unidimensional tiende a favorecer estimadores redundantes, mientras que la combinación con PCD garantiza la complementariedad e independencia estadística entre estimadores.
...
* Esto se debe a que cada cliente selecciona un subconjunto personalizado de árboles que complementa de forma óptima su distribución de datos particular. S7 (`s7_perclient_f1_pcd`) maximiza esta sinergia, logrando un Accuracy medio de **0.8953** frente al **0.8915** de S4 (`s4_global_f1_pcd`).

### 3.3. Comportamiento y Justificación Teórica de S8 (Global Attribute Roulette)
A primera vista, el rendimiento de las variantes S8 (~0.845 Accuracy) podría parecer desfavorable al situarse justo por debajo de la línea base aislada (0.858 Accuracy). Sin embargo, un análisis detallado de la arquitectura de la red revela una ventaja competitiva crucial:
...
* **Sostenibilidad de la comunicación**: S8 retiene el **98.5% del rendimiento** de la línea base aislada (`local_isolation`), lo que la valida como una opción sumamente robusta en escenarios donde los canales de comunicación imponen restricciones severas de transferencia de información.
...
### 3.4. Ganancia de Exactitud sobre la Línea Base Aislada
* **Mejora Absoluta**: La estrategia óptima **S7** eleva el accuracy del 85.78% (línea base aislada) al **89.53%** (mejora absoluta de **+3.75%**). Esta ganancia es muy representativa en datasets de clases complejas como *Vowel* (+13.36% de accuracy sobre el local) y *Glass* (+6.69% sobre el local).

### 3.5. Impacto de la Diversidad del Ensamble (PCD) y el Colapso de Diversidad
* **El Máximo Teórico de Diversidad (S1)**: La estrategia simple de agregación **S1** alcanza la diversidad PCD más alta (**0.5974**). Esto es matemáticamente esperable: al agregar sin discriminación los árboles generados de manera independiente en entornos locales diferenciados, el ensamble retiene la máxima heterogeneidad. Sin embargo, esta alta diversidad no viene acompañada del mejor desempeño (Accuracy de 0.8809), ya que incorpora estimadores redundantes o de baja calidad.
* **El Colapso de Diversidad por Selección Basada únicamente en Precisión (S2, S3)**: Cuando se aplican estrategias de selección basadas únicamente en el rendimiento predictivo individual (como **S2** que usa *Accuracy* o **S3**), la diversidad del ensamble decae. Por ejemplo, **S2** experimenta una caída en PCD a **0.5779** (quedando incluso por debajo del entrenamiento aislado de control, **0.5806**). Este colapso se explica porque la selección basada en métricas unidimensionales tiende a elegir árboles que cometen errores idénticos, reduciendo la complementariedad del ensamble.
* **Optimización Híbrida como Mecanismo de Regularización (S4, S7)**: Al introducir la métrica combinada en el criterio de selección (estrategias **S4** y **S7**), la diversidad se recupera sustancialmente (**0.5823** y **0.5867**), al mismo tiempo que el rendimiento predictivo alcanza su máximo histórico. Esto demuestra que la optimización híbrida actúa como un regularizador del ensamble: penaliza a los estimadores redundantes (aunque tengan buen rendimiento individual) y promueve aquellos que aportan conocimiento novedoso al bosque colectivo.
...
* **PF Centralizado como Cota Superior (Upper Bound)**: El modelo *PF Centralizado* representa el rendimiento máximo teóricamente posible al entrenar sin restricciones de privacidad de datos, logrando un Accuracy medio de **0.9378**.
* **Capacidad de Recuperación del Framework Federado**: La estrategia óptima **S7** recupera el **95.5% del Accuracy** respecto al modelo centralizado. En datasets más estables como *Iris*, *Optdigits*, *Pendigits* y *Spambase*, la brecha de rendimiento absoluto entre el enfoque federado y el centralizado es inferior al **1.8%**, lo que demuestra la robustez del framework.
...
* **Vowel (Ganancia: +13.73% Accuracy)**:
  * *Local Isolation*: 0.7226 | *S5 (Best)*: 0.8599
...
* **Glass (Ganancia: +6.85% Accuracy)**:
  * *Local Isolation*: 0.6595 | *S3 (Best)*: 0.7280
  * *Interpretación*: Un dataset pequeño (214 instancias) y altamente desbalanceado. El entrenamiento aislado sufre de un sesgo extremo. Las estrategias basadas en el servidor mitigan este comportamiento al ponderar de forma justa el desempeño generalizable.
* **Molecular (Ganancia: +7.52% Accuracy)**:
  * *Local Isolation*: 0.7645 | *S7 (Best)*: 0.8397
...
* **Car (Ganancia: +3.63% Accuracy)**:
  * *Local Isolation*: 0.9251 | *S2 (Best)*: 0.9614
...
* **Sonar (Comportamiento atípico: S1 es el mejor con 0.7705)**:
  * *Local Isolation*: 0.7529 | *S1 (Best)*: 0.7705 | *S7*: 0.7598
...
* **Spambase (Ganancia moderada: +0.80% Accuracy)**:
  * *Local Isolation*: 0.9325 | *S2 (Best)*: 0.9405
...
## 5. Análisis de Significancia Estadística (Test de Friedman y Ajuste de Holm)

Para validar si las diferencias observadas en las métricas predictivas (Exactitud / Accuracy) y de diversidad (PCD) son estadísticamente significativas, se llevó a cabo un análisis formal en dos etapas, de acuerdo con las recomendaciones metodológicas de Demšar (2006) para la comparación de clasificadores sobre múltiples conjuntos de datos. Se evaluaron las 12 estrategias federadas frente al modelo de control centralizado (*PF Centralizado*).

### 5.1. Diseño Experimental y Metodología Estadística
...
* **Estrategias ($k = 13$)**:
  * **Baseline Centralizado**: `PF Cent.` (Proactive Forest Centralizado - Control).
  * **Estrategias Federadas Directas**: `S1`, `S2`, `S3`, `S4`, `S5`, `S6`, `S7`.
  * **Estrategias Federadas con Ruleta (Variantes S8)**: `S8_Mean`, `S8_Weighted`, `S8_Median`, `S8_Consensus`, `S8_PCD`.

Dado que las métricas de rendimiento en aprendizaje automático no suelen cumplir con los supuestos de normalidad y homocedasticidad requeridos por las pruebas paramétricas (como ANOVA), se adopta un **enfoque estadístico no paramétrico**.
...
graph TD
    ...
    F --> H[Comparación Pareada contra PF Cent. (Holm)]
    H --> G
```
...
1. **Hipótesis Nula ($H_0$)**: Todas las estrategias tienen un rendimiento equivalente en promedio.
...
3. **Cálculo del Estadístico**:
   Los valores de rendimiento para cada base de datos se ordenan por rangos de $1$ a $k$ (donde $1$ representa el peor rendimiento y $k$ el mejor). El estadístico de Friedman se distribuye aproximadamente como una distribución chi-cuadrado ($\sigma^2$) con $k - 1$ grados de libertad:
   $$\chi_F^2 = \frac{12N}{k(k+1)} \left[ \sum_{j=1}^k R_j^2 - \frac{k(k+1)^2}{4} \right]$$
   Donde $R_j$ es la suma de los rangos para la estrategia $j$-ésima, $N=10$ y $k=13$ (grados de libertad $k-1 = 12$).
4. **Criterio de Decisión**: Si el p-valor es menor que $\alpha = 0.05$, se rechaza $H_0$ y se procede a la Fase 2.

#### Fase 2: Análisis Post-hoc con Ajuste de Holm
Esta segunda fase está estrictamente condicionada al rechazo de la hipótesis nula en la Fase 1. Si la prueba de Friedman no es significativa ($p \ge 0.05$), no se permite proceder al análisis post-hoc, deteniéndose el flujo para evitar el incremento de falsos positivos.

Se realiza una comparación post-hoc enfocada en evaluar cada estrategia individualmente contra el baseline de control centralizado (`PF Cent.`). Para controlar la Tasa de Error por Familia (FWER), se aplica el **procedimiento secuencial descendente de Holm** (Holm, 1979) para ajustar los p-valores crudos de las $m = k - 1$ comparaciones ($12$ comparaciones en este estudio). Cada p-valor crudo $p_i$ ordenado secuencialmente se evalúa contra $\alpha_i = \frac{\alpha}{m - i + 1}$. El p-valor ajustado de Holm se calcula como:
$$p^{\text{adj}}_i = \min\left(1, \max_{j \le i} \left( (m - j + 1) \cdot p_j \right)\right)$$

---

### 5.2. Análisis de Exactitud (Accuracy)

* **Test de Friedman (Global)**:
  * Estadístico $\chi_F^2$: **91.9977**
  * $p$-valor global: **$2.02 \times 10^{-14}$**
  * **Conclusión**: **Existen diferencias globales significativas** ($p < 0.05$) en exactitud. Se autoriza la ejecución del análisis post-hoc.
* **Resultados del Post-hoc de Holm (PF Cent. como Control)**:
  * La comparación pareada contra el *PF Centralizado* arrojó un $p$-valor crudo de **0.001953** para todos los tratamientos federados.
  * El ajuste de Holm para las $m=12$ comparaciones resulta en un $p$-valor ajustado de **0.023438** ($12 \times 0.001953125$).

| Estrategia | Mean Accuracy | $p$-value (raw) | $p$-value (Holm) | Diferencia de Media | Significativa (Holm, p < 0.05) |
| :--- | :---: | :---: | :---: | :---: | :---: |
| **PF Cent. (Control)** | **0.9378** | **-** | **-** | **-** | **Control** |
| `S7` | 0.8953 | 0.001953 | 0.023438 | -0.0425 | Sí |
| `S4` | 0.8915 | 0.001953 | 0.023438 | -0.0463 | Sí |
| `S6` | 0.8910 | 0.001953 | 0.023438 | -0.0468 | Sí |
| `S5` | 0.8909 | 0.001953 | 0.023438 | -0.0469 | Sí |
| `S2` | 0.8891 | 0.001953 | 0.023438 | -0.0487 | Sí |
| `S3` | 0.8888 | 0.001953 | 0.023438 | -0.0490 | Sí |
| `S1` | 0.8809 | 0.001953 | 0.023438 | -0.0569 | Sí |
| `S8_PCD` | 0.8457 | 0.001953 | 0.023438 | -0.0921 | Sí |
| `S8_Consensus` | 0.8454 | 0.001953 | 0.023438 | -0.0924 | Sí |
| `S8_Mean` | 0.8453 | 0.001953 | 0.023438 | -0.0925 | Sí |
| `S8_Weighted` | 0.8453 | 0.001953 | 0.023438 | -0.0925 | Sí |
| `S8_Median` | 0.8449 | 0.001953 | 0.023438 | -0.0929 | Sí |

* **Conclusión**: Se confirma formalmente que el modelo centralizado es estadísticamente superior en exactitud a todas las configuraciones federadas ($p < 0.05$), consolidando científicamente la existencia del costo de privacidad.

---

### 5.3. Análisis de Diversidad Estructural (PCD)

* **Test de Friedman (Global)**:
  * Estadístico $\chi_F^2$: **18.7281**
  * $p$-valor global: **0.0953**
  * **Conclusión**: **NO existen diferencias globales significativas** ($p = 0.0953 \ge 0.05$) en cuanto a diversidad estructural PCD.
  
> [!NOTE]
> De acuerdo con la condición de parada metodológica, dado que la prueba global de Friedman no es significativa ($p \ge 0.05$), se detiene la evaluación estadística de esta métrica. No se autoriza la aplicación del post-hoc de Holm para evitar inflar la tasa de falsos positivos.

* **Interpretación**: No existe evidencia estadística suficiente bajo un nivel de confianza del 95% para afirmar que alguna de las estrategias de agregación federada altere o degrade de forma generalizable la diversidad estructural (PCD) del bosque en comparación con el modelo centralizado de referencia. Esto es una conclusión científica positiva, ya que valida que la descentralización federada no perjudica la heterogeneidad intrínseca de los estimadores del bosque colaborativo.
