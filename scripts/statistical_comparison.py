"""Script to run statistical Friedman and Wilcoxon comparison tests."""
...
def run_statistical_comparison() -> None:
    """Loads benchmark CSV and runs Friedman/Wilcoxon tests against baseline."""
    ...
    try:
        ...
        df_pivot = df.pivot_table(
            index="dataset", columns="strategy", values="f1_mean"
        )
        ...
        if p_value < 0.05:
            print(
                "RESULTADO: Existen diferencias significativas entre las "
                "estrategias (p < 0.05)."
            )
        else:
            print(
        ...
        for col in df_pivot.columns:
            if col == "Nayma_PF_Centralized":
                continue

            # Wilcoxon signed-rank test
            _, w_p = stats.wilcoxon(
                df_pivot["Nayma_PF_Centralized"], df_pivot[col]
            )
            comparisons.append(
                {
                    "Strategy": col,
                    "p-value": w_p,
                    "Mean_F1": df_pivot[col].mean(),
                }
            )
        ...
        print(
            f"Tu mejor estrategia federada es: {best_fed['Strategy']} "
            f"(Mean F1: {best_fed['Mean_F1']:.4f})"
        )

        significant_diffs = comp_df[comp_df["p-value"] < 0.05]
        if not significant_diffs.empty:
            print(
                f"Hay {len(significant_diffs)} estrategias con diferencias "
                f"significativas vs la Tesis."
            )
        else:
            print(
                "No hay diferencias estadísticamente significativas vs la "
                "Tesis (tus resultados son comparables)."
            )
