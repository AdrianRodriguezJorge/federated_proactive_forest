"""Benchmark de un solo fold solo para las estrategias S9.

Ejecuta las variantes S9 del experimento definitivo sin la estrategia
'local_isolation' y sin validación cruzada. Solo calcula F1, accuracy y PCD.
"""
...
# ==========================================================================
# CONFIGURACIÓN DEL EXPERIMENTO S9
# ==========================================================================
REPETITIONS = 1
TEST_SIZE = 0.1
N_CLIENTS = 3
...
RESULTS_FILE = "results/results_final_benchmark_s9_single_fold.csv"
...
def run_single_strategy(
    ...
    for split in precomputed_splits:
        ...
        if strategy.startswith("s9_"):
            ...
            raise ValueError(f"Estrategia no soportada en el experimento S9: {strategy}")
...
def run_s9_benchmark() -> None:
    print("=== BENCHMARK DE UN SOLO FOLD: SOLO S9 ===")
    print(f"Repeticiones: {REPETITIONS} | Single Fold | Clientes: {N_CLIENTS}")
    ...
    for rep in range(1, REPETITIONS + 1):
        for ds_name in DATASETS:
            if all((rep, strat, ds_name) in completed for strat in S9_STRATEGIES):
                continue
            ...
            pending_strategies = [
                s for s in S9_STRATEGIES if (rep, s, ds_name) not in completed
            ]
            if not pending_strategies:
                continue

            print(f"  Ejecutando {len(pending_strategies)} estrategias S9 para {ds_name}...")

            for strat in pending_strategies:
                r = run_single_strategy(strat, ds_name, rep, precomputed_splits)
    ...
    print(f"\n=== BENCHMARK S9 FINALIZADO. Resultados en {RESULTS_FILE} ===")
